...
def load_data(filename):
    ...
    with open(filename, encoding="utf-8") as f:
        ...
        next(reader)
        for row in reader:
            date = "Jun" if row[10] == "June" else row[10]
            ...
            evidence.append([int(row[0]), float(row[1]), int(row[2]), float(row[3]), int(row[4]),
            float(row[5]), float(row[6]), float(row[7]), float(row[8]), float(row[9]),
            int(month), int(row[11]), int(row[12]), int(row[13]), int(row[14]),
            int(visitor), int(weekend)])
    ...
    for x in zip(labels, predictions):
        if x[0] == x[1]:
            ...
            if x[0] == 1:
                ...
                incorrectNegative += 1
    totalPositive = correctPositive + incorrectPositive
    totalNegative = correctNegative + incorrectNegative
